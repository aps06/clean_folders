        ...
        if files != []:
            ...
            for file in files:
                ...
                if parameters[0] in ['images', "video", "documents", "audio", 'other']:
                    shutil.move(os.path.join(path_folder, file), os.path.join(create_path, f'{parameters[1]}{parameters[2]}'))

                elif parameters[0] == 'archives':
                    os.chdir(path_folder)
                    try:
                        if parameters[2].lower() != '.gz':
                            shutil.unpack_archive(file, os.path.join(create_path, parameters[1]))
                        else:
